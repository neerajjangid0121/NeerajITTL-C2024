...
public class TumblrApiParser {
    ...
    public static void parseImageUrls(String jsonText, int start, int end) {
        String[] posts = jsonText.split("\"photos\":\\["); // Splitting JSON to individual posts
        ...
        for (int i = 1; i < posts.length && postNumber <= end; i++) {
            ...
            for (int j = 1; j < images.length; j++) {
                String imageUrl = images[j].split("\"")[0]; // Extract the image URL from regex
                imageUrl = imageUrl.replace("\\", ""); // removing escape characters
